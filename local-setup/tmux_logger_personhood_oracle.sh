...
if tmux has-session -t integritee_logger_three_nodes ; then
  ...
  tmux new-session -d -s integritee_logger_three_nodes \; \
    ...
    send-keys -t integritee_logger_three_nodes:0.2 "printf '\033]2;ENCOINTER NODE\033\\'; clear" C-m \; \
    send-keys -t integritee_logger_three_nodes:0.2 'tail -f ../log/node2.log' C-m \; \
    send-keys -t integritee_logger_three_nodes:0.3 "printf '\033]2;SUBSTRATE NODE TEMPLATE\033\\'; clear" C-m \; \
    send-keys -t integritee_logger_three_nodes:0.3 'tail -f ../log/node3.log' C-m \; \
    send-keys -t integritee_logger_three_nodes:0.4 "printf '\033]2;TEE PERSONHOOD ORACLE\033\\'; clear" C-m \; \
